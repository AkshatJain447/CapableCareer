...
var jobList = [];
var jobContainer = document.querySelector("#jobContainer");

function fillJobContainer(){
  if (jobList.length === 0) {
    return;
  }
  ...
  jobList.forEach(job=>{
    ...
    jobCard.innerHTML = `
    <h5 class="jobcardhead">Job Position: ${job.jobTitle}</h5>
    <div class = "jobCardC">
      <div class="jobcardA">
        <ul>
          <li><p class="jobcardheading">Job Id:</p> ${job.JobId}</li>
          <li><p class="jobcardheading">Company/Organization:</p> ${job.Company}</li>
          <li><span class="jobcardheading">Job Description:</span> ${job.Description}</li>
        </ul>
      </div>
      <div class="jobcardB">
        <ul>
          <li><p class="jobcardheading">Application Deadline:</p> ${job.Deadline}</li>
          <li><p class="jobcardheading">Vacancy:</p> ${job.Vacancies}</li>
          <li><p class="jobcardheading">Eligibility:</p> ${job.EligibilityCriteria}</li>
          <li><p class="jobcardheading">Location:</p> ${job.JobLocation }</li>
          <li><button class="jobCardBtn">Apply Now</button></li>
        </ul>
      </div>
    </div>`;
