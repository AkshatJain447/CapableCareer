...
function fillcourseContainer(){
  ...
  courseList.forEach(course=>{
    ...
    <div class="courseCardA">
      <iframe width="355" height="220" src="${course.src}" frameborder="0" allowfullscreen></iframe>
      <div>
        <ul>
          <li><span class="courseCardHeading">Course Title:</span> ${course.title}</li>
          <li><span class="courseCardHeading">Duration:</span> ${course.duration}</li>
          <li><span class="courseCardHeading">Course Description:</span> ${course.desc}</li>
        </ul>
      </div>
    </div>`;
...
function GetAllDataOnce() {
  get(child(ref(db), "CourseList")).then((snapshot) => {
    snapshot.forEach((childSnapshot) => {
      courseList.push(childSnapshot.val());
    });
    fillcourseContainer();
  });
}
